...
\maketitle
\begin{abstract}
\noindent Ownership changes are common in U.S. nursing homes, yet their implications for care production remain unclear. Using administrative panel data from the Centers for Medicare and Medicaid Services (CMS) from 2017 to 2024, we study how ownership changes affect nurse staffing, the core input to nursing home care production. We identify control-changing ownership changes and estimate effects using a staggered-adoption difference-in-differences event-study design. We find that ownership changes lead to persistent declines in staffing, concentrated in registered nurses, certified nursing assistants, and total nursing hours per patient day, with little adjustment in licensed practical nurse staffing. Importantly, effects are substantially larger among facilities that are not chain-affiliated and before the COVID-19 pandemic. These patterns indicate that staffing is a central adjustment margin following ownership changes in a regulated setting with limited short-run pricing flexibility, suggesting that consolidation can affect resident welfare through internal production decisions even when facilities remain open. \\
\vspace{0in}\\
...
To answer the research question, we use Centers for Medicare and Medicaid Services (CMS) date to construct a panel of nursing homes across the 48 contiguous U.S. states from January 2017 through June 2024. We measure staffing outcomes using hours per patient day (HPPD) for RNs, LPNs, CNAs, and total nursing hours. We estimate treatment effects using a staggered adoption difference-in-differences event study design. The results show persistent declines in nursing staffing following ownership changes, concentrated among RNs, CNAs, and total nursing hours, with heterogeneity by chain affiliation and the COVID-19 pandemic.
...
Table~\ref{tab:pretrend-wald-tests} reports the results of our Wald test. In the ``2 Year Full Pre-Window'' specification, which includes the full set of pre-treatment months ($\tau=-24$ to $\tau=-2$) and uses $\tau=-1$ as the reference period, the joint Wald tests reject the null of no pre-trends for several outcomes, particularly for RN and total staffing in both levels and logs. This indicates that staffing shows change effects in the periods leading up to the recorded ownership transition when the months immediately preceding the change are included in the pre-window, consistent with anticipatory adjustments around the transition date. Administrative ownership dates may lag operational changes, so staffing changes can begin before the recorded month.

We therefore also report ``donut'' specifications that exclude the three months immediately preceding ownership changes ($\tau\in\{-3,-2,-1\}$) and use $\tau=-4$ as the reference period. This ``donut'' is aimed at mitigating this timing mismeasurement with administrative dates and operational changes. Relative to the full pre-window tests, the donut specifications yield substantially weaker evidence of pre-trend violations for several outcomes, with joint tests that are no longer statistically distinguishable from zero in the shorter one-year donut window. Overall, these results suggest that the months immediately before the recorded change month are likely contaminated by anticipation, and that excluding $\tau\in\{-3,-2,-1\}$ improves the plausibility of the parallel trends assumption.
...
We find that ownership changes are followed by persistent reductions in nurse staffing, concentrated in RN hours, CNA hours, and total nursing hours per patient day. In a regulated sector with limited short-run pricing flexibility, staffing is one of the most immediately adjustable margins, and changes in staffing provide a direct window into how new owners reshape internal production decisions. The patterns in the event-study estimates, together with the robustness of results across alternative specifications, suggest that these changes reflect sustained operational adjustments rather than short changes around the ownership change.

\subsection{No declines in LPN hours}

One interesting result we find is that LPN staffing shows little to no decline following ownership change, even as RN and CNA hours fall. This is consistent with substitution across nurse types arising from differences in training, scope of practice, and how tasks are organized within nursing homes. RNs perform higher-skill clinical and supervisory functions, while CNAs provide the majority of hands-on daily care \citep{lin_2014}. LPNs occupy an intermediate role: they can perform a set of routine clinical and medication-management tasks that overlap with RN responsibilities, and they can also cover some tasks that would otherwise require CNA time through direct resident contact in day-to-day work \citep{lin_2014}.

LPNs provide the most feasible substitution margin when new owners seek to decrease labor costs without triggering large disruptions in observable service provision. CNAs generally cannot substitute for RNs on clinical oversight and licensed tasks, and RNs are typically not used as substitutes for CNA-level direct-care tasks because of their high wage costs and the organization of nursing home production. LPNs, by contrast, can substitute in both directions in the short run: facilities can economize on RN hours by shifting certain routine clinical responsibilities toward LPNs, while also relying on LPNs to maintain basic clinical coverage when CNA hours are reduced. This type of compositional adjustment is consistent with evidence that regulation and resource constraints can induce facilities to alter staffing composition rather than reduce all staffing categories proportionally \citep{bowblis_ghattas_2017}. More broadly, the stability of LPN hours suggests that ownership changes may operate less through uniformly reducing staffing and more through reallocating tasks across skill categories, with LPNs acting as the residual workforce that supports continuity of operations.

\subsection{Are the staffing reductions meaningful?}

Although the estimated staffing reductions are statistically precise, an important question is whether they are large in economic or clinical terms. Benchmarking the estimates against baseline staffing levels provides context. In our sample, from table~\ref{tab:sumstats} mean staffing levels are 0.427 RN HPPD, 0.796 LPN HPPD, 2.100 CNA HPPD, and 3.324 total HPPD.
In the baseline TWFE specification, an ownership change reduces RN staffing by 0.032 HPPD, CNA staffing by 0.054 HPPD, and total staffing by 0.086 HPPD, with no statistically significant change in LPN staffing.

These effects correspond to approximately a 7.5\% decline in RN hours ($0.032/0.427$), a 2.6\% decline in CNA hours ($0.054/2.100$), and a 2.6\% decline in total nursing hours ($0.086/3.324$). The log specifications yield closely aligned percentage changes: RN log(HPPD) falls by 0.092 (about 9\%), CNA log(HPPD) falls by 0.028 (about 2.8\%), and total log(HPPD) falls by 0.026 (about 2.6\%). The consistency between level-based benchmarks and log-based estimates strengthens the interpretation that staffing declines are modest in proportional terms for total hours, with a larger proportional reduction concentrated in RN time.

To interpret magnitudes intuitevely, note that 0.032 RN HPPD corresponds to about $0.032 \times 60 \approx 1.9$ fewer RN minutes per resident-day, while 0.054 CNA HPPD corresponds to about $3.2$ fewer CNA minutes per resident-day. The 0.086 reduction in total HPPD corresponds to about $5.2$ fewer total nursing minutes per resident-day. These per-resident changes are small in isolation, and one reason they are estimated with high statistical significance is the very large panel of facility-month observations. In other words, statistical precision here reflects sample size and repeated observations, not necessarily large changes in day-to-day care time.

In a facility with roughly 100 residents, this corresponds mechanically to about $0.086 \times 100 = 8.6$ fewer nursing staff-hours per day. However, in medium and large facilities this reduction is typically diffused across many residents and across multiple shifts. Put differently, average minutes per resident-day can decline while the realized resident-level impact remains difficult to detect if facilities reallocate tasks (particularly toward LPNs) or trim time in ways that do not affect measured outcomes. By contrast, in smaller facilities with fewer staff on duty per shift and less slack, comparable reductions may be more likely to bind and to concentrate into noticeable gaps in coverage for particular residents. These considerations underscore why statistically precise estimates of small average changes do not necessarily imply large welfare effects, and why linking ownership transitions to resident outcomes is a natural next step for evaluating whether these staffing reductions reflect efficiency-enhancing reorganization or quality-reducing cost containment.
...
Overall, ownership transitions are associated with persistent staffing reductions concentrated in RN and CNA hours, while LPN staffing remains stable, consistent with substitution toward an intermediate-skilled workforce. The magnitudes are modest in proportional terms for total staffing but larger for RN time, and the welfare relevance depends on whether these changes reflect efficiency-enhancing reallocation or quality-reducing cost containment. Future work linking ownership transitions to resident outcomes and broader operational measures would help clarify the mechanisms and implications of consolidation in nursing home markets.
