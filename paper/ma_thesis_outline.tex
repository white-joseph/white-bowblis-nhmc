...
Make Paragraph 2 of the introduction institutionally concrete: Medicaid as the dominant payer with state-administered per-diems, Medicare's PDPM rates for short-stay post-acute residents. Explain why administered pricing pushes an acquirer's optimization onto input and census margins rather than price.
...
\medskip
\noindent\textbf{Wage structure} RN hourly wages run roughly double CNA wages. This is essential context for interpreting the main result: a reduction of 88.6 RN hours generates substantially larger cost savings than a reduction of 75.5 CNA hours, which is why cutting the smallest staffing category can be the rational margin for a cost-conscious new owner (I think raw hour decreases are more important than in my original paper, given the fact that occupancy rate increases simultaneously with the HPRD decrease).
...
\medskip
\noindent Third paragraph: RN hours fall roughly 8.4 percent against CNA at 1.8 percent and LPN flat; RNs supply approximately 60 percent of the total hour reduction ($-88.6$ of $-148.2$ monthly hours) from the smallest staffing category. Interpret through Section~\ref{sec:background-staffing}: the wage differential makes RN hours the largest cost saving per hour removed, and RN functions are the least immediately visible. 
...
First paragraph: pull the three outcome families into one interpretation; new owners reduce nursing inputs disproportionately at the RN margin while filling beds, and measured resident outcomes hold steady even as preventive process measures worsen. Second paragraph: why the skill-mix pattern favors a reorganization-with-selective-disinvestment reading over both pure cost-cutting and pure efficiency stories. Third paragraph: economic magnitudes benchmarked against baseline levels (monthly hours removed per facility, RN share of the reduction, occupancy gain relative to baseline spare capacity, translated into approximate annual labor cost savings if the wage figures support it). Fourth paragraph: heterogeneity (slack-capacity concentration, chain transitions, pandemic period) as evidence that capacity and constraints shape feasible adjustment. Fifth paragraph: limitations---residual timing measurement error; non-random transaction timing and coincident unobserved shocks; quality measures omitting mortality, hospitalization, and resident experience; HCRIS annual frequency limiting event-study resolution on business-model outcomes; inability to observe task allocation directly, which is what the mechanism argument would require; and a post-transition window that may be too short to detect slowly accruing harms.

\section{Conclusion} \label{sec:conclusion}

Three short paragraphs: restate the skill-mix finding and the joint staffing--composition--quality pattern; restate the three contributions with both measurement contributions foregrounded; close on open questions---which acquisitions generate efficiency-enhancing reorganization versus quality-reducing disinvestment, whether the answer is predictable from pre-transition slack, ownership structure, or acquirer type, and whether the preventive-care erosion translates into resident harm over longer horizons than this panel observes.
