...
We estimate event-study models to follow the evolution of staffing and quality outcomes before and after ownership change. Let \(Y_{ip}^{m}\) denote outcome \(m\) for facility \(i\) in period \(p\). For staffing outcomes, \(p\) indexes calendar months and \(m\) includes RN, LPN, CNA, and total HPRD. For quality outcomes, \(p\) indexes calendar quarters and \(m\) includes the selected CMS quality measures. We estimate:

\begin{equation}
\label{eq:event-study}
Y_{ip}^{m}
=
\sum_{\tau \in \mathcal{K}^{m}}
    \beta_{\tau}^{m}
    \mathbf{1}\!\left( \text{EventTime}_{ip}^{m} = \tau \right)
    \cdot \text{Treat}_{i}
+
X_{ip}^{m}\gamma^{m}
+
...
The variable \(\text{EventTime}_{ip}^{m}\) measures time relative to the ownership change. For staffing outcomes, event time is measured in months. For quality outcomes, event time is measured in quarters. \(X_{ip}^{m}\) includes time-varying facility characteristics, while \(\alpha_i^{m}\) and \(\lambda_p^{m}\) denote facility and calendar-period fixed effects, respectively. The coefficients \(\beta_{\tau}^{m}\) describe how outcome \(m\) evolves before and after ownership change relative to the omitted reference period.

Because staffing and quality outcomes are measured at different frequencies, the included event-time indicators differ across outcome families. For staffing outcomes, the event-study window is \([-24,24]\) months. In the preferred staffing specification, we omit \(\tau=-4\) as the reference period and exclude \(\tau\in\{-3,-2,-1\}\) to remove the immediate pre-transition period most likely to be contaminated by anticipation. Thus,

\[
\mathcal{K}^{S}=\{-24,\ldots,24\}\setminus\{-4,-3,-2,-1\}.
\]

For quality outcomes, the event-study window is \([-8,8]\) quarters. In the preferred quality specification, we omit 
\(\tau=-1\) as the reference period and exclude \(\tau=0\), the quarter containing the ownership change. Thus,

\[
\mathcal{K}^{Q}=\{-8,\ldots,8\}\setminus\{-1,0\}.
\]

For staffing outcomes, we estimate models in both levels and logs. Log specifications allow the coefficients to be interpreted as approximate percentage changes and reduce sensitivity to differences in baseline staffing levels across facilities. Log specifications are omitted when staffing is zero.

We also estimate average post-change models that summarize the event-study results with a single coefficient:

\begin{equation}
\label{eq:twfe-post}
Y_{ip}^{m}
=
\delta^{m}\textit{Post}_{ip}^{m}
+
X_{ip}^{m}\gamma^{m}
+
\alpha_i^{m}
...
The indicator \(\textit{Post}_{ip}^{m}\) equals one in periods after the ownership change and zero otherwise. For staffing outcomes, this indicator is defined at the facility-month level. For quality outcomes, it is defined at the facility-quarter level. The coefficient \(\delta^{m}\) captures the average post-ownership-change difference in outcome \(m\), relative to the pre-change period, after absorbing facility fixed effects and common calendar-time shocks.

For quality outcomes, we estimate specifications both with and without RN, LPN, and CNA HPRD controls. Because staffing is itself affected by ownership change, the staffing-controlled quality specifications are not interpreted as preferred estimates of the total causal effect of ownership change on quality. Instead, they are used descriptively to assess whether the estimated quality responses are weakened after conditioning on measured staffing inputs.

\subsubsection{Parallel Trends}

The identifying assumption underlying our model design is that, absent an ownership change, staffing outcomes in treated facilities would have followed the same trends as those in not-yet treated and untreated facilities. We assess the plausibility of this assumption by examining estimated coefficients on the pre-treatment event-time indicators and by conducting joint Wald tests of whether the pre-treatment coefficients are jointly equal to zero.
