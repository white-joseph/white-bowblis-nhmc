...
\footnotesize
\item \textit{Notes:} Each cell reports the coefficient on \textit{post} with two-way clustered standard errors (by facility and month) in parentheses. Panel~A reports levels (HPPD); Panel~B reports logs (HPPD). Sample: \textit{Without anticipation} ($N_{\mathrm{levels}}=628,107;\ N_{\mathrm{logs}}=628,107$).
